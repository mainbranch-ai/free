#!/usr/bin/env bash
# detect-tools.sh — Probe research tools not yet tracked in .mainbranch/config.yaml
# Called once per /think session. Results should be written back to config.yaml.

CONFIG=".mainbranch/config.yaml"
...
[ "$GEMINI_STATUS" = "null" ] && {
  [ -z "$GOOGLE_API_KEY" ] && [ -f "$HOME/.config/mainbranch/env.sh" ] && source "$HOME/.config/mainbranch/env.sh"
  [ -n "$GOOGLE_API_KEY" ] \
...
[ "$GROK_STATUS" = "null" ] && {
  ...
  [ -z "$XAI_API_KEY" ] && [ -f "$HOME/.config/mainbranch/env.sh" ] && source "$HOME/.config/mainbranch/env.sh"
