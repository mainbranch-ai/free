...
Tool detection is handled automatically by `/think` via `.mainbranch/config.yaml` -- see SKILL.md for details.

### Claude Code Env Var Caveat

Claude Code sessions may not inherit env vars from your shell profile (`~/.zshrc`, `~/.bashrc`). If `XAI_API_KEY` isn't in the environment, the skill checks for and sources `~/.config/mainbranch/env.sh` before giving up. If you store API keys in a different location, update the detection script in SKILL.md or export the key in `~/.config/mainbranch/env.sh`.
